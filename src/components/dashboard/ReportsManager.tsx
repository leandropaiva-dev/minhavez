...
import { Calendar, Users, Clock, CheckCircle, XCircle, TrendingUp, TrendingDown, Loader } from 'react-feather'
...
export default function ReportsManager({ businessId }: ReportsManagerProps) {
  ...
  const fetchReportData = useCallback(async () => {
    ...
    const queueQuery = supabase
      .from('queue_entries')
      .select('id, status, joined_at, called_at, attended_at, completed_at, party_size')
    ...
    const reservationsQuery = supabase
      .from('reservations')
      .select('id, status, reservation_date, reservation_time, created_at, updated_at, party_size')
      .eq('business_id', businessId)
    ...
    const currentEntries: { date: string; status: string; waitTime: number; type: 'queue' | 'reservation'; hour: number; weekday: number; partySize: number }[] = []
    ...
    if (typeFilter === 'all' || typeFilter === 'queue') {
      queueData?.forEach(entry => {
        ...
        const waitTime = entry.called_at && entry.joined_at
          ...
          : 0
        currentEntries.push({
          date: joinedDate.toISOString().split('T')[0],
          status: entry.status,
          waitTime,
          type: 'queue',
          hour: joinedDate.getHours(),
          weekday: joinedDate.getDay(),
    ...
    if (typeFilter === 'all' || typeFilter === 'reservation') {
      reservationsData?.forEach(entry => {
        const reservationDate = new Date(entry.reservation_date)
        currentEntries.push({
          date: entry.reservation_date,
          status: entry.status,
          waitTime: 0,
          type: 'reservation',
          hour: parseInt(entry.reservation_time.split(':')[0]),
          weekday: reservationDate.getDay(),
          partySize: entry.party_size || 1,
        })
      })
    }
    ...
    const prevEntries: { status: string; waitTime: number; partySize: number }[] = []
    ...
    if (typeFilter === 'all' || typeFilter === 'queue') {
      prevQueueData?.forEach(entry => {
        const waitTime = entry.called_at && entry.joined_at
          ? (new Date(entry.called_at).getTime() - new Date(entry.joined_at).getTime()) / 60000
          : 0
        prevEntries.push({ status: entry.status, waitTime, partySize: entry.party_size || 1 })
      })
    }

    if (typeFilter === 'all' || typeFilter === 'reservation') {
      prevReservationsData?.forEach(entry => {
        prevEntries.push({ status: entry.status, waitTime: 0, partySize: entry.party_size || 1 })
      })
    }
    ...
    currentEntries.forEach(e => {
      hourMap.set(e.hour, (hourMap.get(e.hour) || 0) + 1)
    })
    ...
    // Find most popular weekday
    const weekdayMap = new Map<number, number>()
    currentEntries.forEach(e => {
      weekdayMap.set(e.weekday, (weekdayMap.get(e.weekday) || 0) + 1)
    })
    const mostPopularDay = Array.from(weekdayMap.entries())
      .sort((a, b) => b[1] - a[1])[0]
    const diaSemanaPopular = mostPopularDay ? WEEKDAYS[mostPopularDay[0]] : '-'

    // Calculate total attendance time
    const tempoTotalAtendimento = Math.round(waitTimes.reduce((a, b) => a + b, 0))
    ...
    currentEntries.forEach(entry => {
      if (hourlyMap.has(entry.hour)) {
        hourlyMap.set(entry.hour, (hourlyMap.get(entry.hour) || 0) + 1)
      }
    })
    ...
    const weekdayChartData: WeekdayData[] = WEEKDAYS.map((dia, index) => ({
      dia,
      clientes: weekdayMap.get(index) || 0,
    }))
    ...
    // Status distribution
    setStatusData([
      { name: 'Concluídos', value: totalCompletados, color: COLORS.completed },
      { name: 'Cancelados', value: totalCancelados, color: COLORS.cancelled },
    ])
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div data-tutorial="reports-metrics" className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 sm:p-4">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 text-[10px] sm:text-xs mb-0.5">Total Cancelados</p>
          <p className="text-xl sm:text-2xl font-bold text-zinc-900 dark:text-white">{stats.totalCancelados}</p>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 sm:p-4">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 text-[10px] sm:text-xs mb-0.5">Horários de Pico</p>
          <p className="text-xl sm:text-2xl font-bold text-zinc-900 dark:text-white">{stats.horasPico}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 sm:p-4">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 text-[10px] sm:text-xs mb-0.5">Dia Mais Popular</p>
          <p className="text-xl sm:text-2xl font-bold text-zinc-900 dark:text-white">{stats.diaSemanaPopular}</p>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 sm:p-4">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 text-[10px] sm:text-xs mb-0.5">Total Concluídos</p>
          <p className="text-xl sm:text-2xl font-bold text-zinc-900 dark:text-white">{stats.totalCompletados}</p>
        </div>
      </div>
      ...
      <div data-tutorial="reports-charts" className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 sm:p-6">
          <h3 className="text-zinc-900 dark:text-white font-semibold text-sm sm:text-base mb-3 sm:mb-4">
            Clientes por Dia (últimos 14 dias)
          </h3>
          {dailyData.length > 0 ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 sm:p-6">
          ...
          {weekdayData.some(d => d.clientes > 0) ? (
            <div className="w-full overflow-x-auto -mx-2 sm:mx-0">
              <div className="min-w-[300px] px-2 sm:px-0">
                <ResponsiveContainer width="100%" height={220}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg">
            ...
            <span className="text-lg font-bold text-zinc-900 dark:text-white">
              {stats.tempoTotalAtendimento > 60
                ? `${Math.round(stats.tempoTotalAtendimento / 60)}h`
                : `${stats.tempoTotalAtendimento}min`
              }
            </span>
          </div>
          ...
          <div className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-zinc-700 dark:text-zinc-300">Dia Mais Popular</span>
            </div>
            <span className="text-lg font-bold text-zinc-900 dark:text-white">{stats.diaSemanaPopular}</span>
          </div>
        </div>
      </div>
    </div>
